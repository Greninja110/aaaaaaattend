...
import csv
import io
# import pandas as pd  # Temporarily disabled due to Python 3.12 compatibility
...
def handle_uploaded_csv(file, import_type, user):
    ...
    try:
        ...
        if import_type == 'students':
            results = process_student_import(rows, user, import_log)
        elif import_type == 'faculty':
            results = process_faculty_import(rows, user, import_log)
        else:
...
def process_student_import(rows, user, import_log):
    """Process student import from list of dictionaries"""
    try:
        with transaction.atomic():
            ...
            for idx, row in enumerate(rows):
                try:
                    ...
                    dob = None
                    if 'dob' in row and row['dob'] and str(row['dob']).strip():
                        try:
...
def process_faculty_import(rows, user, import_log):
    """Process faculty import from list of dictionaries"""
    try:
        with transaction.atomic():
